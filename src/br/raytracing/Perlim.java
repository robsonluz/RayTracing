...
public class Perlim {
	private static Perlim instance;

	private int p[] = new int[512];

	private static int permutation[] = { 151, 160, 137, 91, 90, 15, 131, 13,
	...
	public static Perlim getInstance() {
		if (instance == null)
			instance = new Perlim();
		return instance;
	}
	...
	private double grad(int hash, double x, double y, double z) {
		...
		double u = h < 8 || h == 12 || h == 13 ? x : y, // INTO 12 GRADIENT
														// DIRECTIONS.
		v = h < 4 || h == 12 || h == 13 ? y : z;
	...
	public double noise(double x, double y, double z) {
		...
		w = fade(z);
		int A = this.p[X] + Y, AA = this.p[A] + Z, AB = this.p[A + 1] + Z, // HASH
																			// COORDINATES
																			// OF
		B = this.p[X + 1] + Y, BA = this.p[B] + Z, BB = this.p[B + 1] + Z; // THE
																			// 8
																			// CUBE
																			// CORNERS,
		...
		return lerp(w, lerp(v, lerp(u, grad(this.p[AA], x, y, z), // AND ADD
				...
				lerp(v, lerp(u, grad(this.p[AA + 1], x, y, z - 1), // CORNERS
						...
						lerp(u, grad(this.p[AB + 1], x, y - 1, z - 1), grad(
								this.p[BB + 1], x - 1, y - 1, z - 1))));
	}
